...
• Так фото сохранится в карточку растения

💾 КОЛЛЕКЦИЯ
• Используй инструменты для управления растениями
• Если пользователь упоминает своё растение — проверь базу
• При добавлении растения — уточняй интервал полива

📔 ЖУРНАЛ УХОДА
• Когда пользователь говорит «удобрил», «пересадил», «обрезал» — логируй через log_care_action
• Типы: полив, удобрение, пересадка, обрезка, лечение, заметка

═══ ФОРМАТ ═══
• Эмодзи для структуры (🌿 🔍 💧 ⚠️ ✅)
• Диагностика: Диагноз → Причина → Лечение → Профилактика
...
TOOLS = [
    {
        "name": "get_plants",
        "description": "Получить список всех растений пользователя",
        "input_schema": {
            "type": "object",
    ...
    {
        "name": "add_plant",
        "description": "Добавить новое растение в коллекцию",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "delete_plant",
        "description": "Удалить растение из коллекции",
        "input_schema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": "Сохранить фото к растению (Telegram file_id)",
        "input_schema": {
            "type": "object",
            "properties": {
                "plant_id": {"type": "integer"},
    ...
    },
    {
        ...
        "description": "Записать действие по уходу: удобрение, пересадка, обрезка, лечение, заметка",
        "input_schema": {
            "type": "object",
            "properties": {
                "plant_id": {"type": "integer"},
                ...
                "action_type": {
                    "type": "string",
                    "enum": ["удобрение", "пересадка", "обрезка", "лечение", "заметка", "полив"],
                },
                "notes": {"type": "string"},
            },
    ...
    },
    {
        ...
        "description": "Получить историю ухода за растением",
        "input_schema": {
            "type": "object",
            "properties": {
                "plant_id": {"type": "integer"},
...
async def execute_tool(name: str, input_data: dict) -> str:
    try:
        if name == "get_plants":
            ...
            return json.dumps({"success": True, "message": "Удалено"})
            ...
            return json.dumps({"success": True, "message": f"Записано: {input_data['action_type']}"})
